...
type Request struct {
	...
	Session          string
	Exclude          []uint
	ExcludeProviders []uint
	ExcludeKeyModels []uint
...
type Candidate struct {
	...
	KeyInflight         int                 `json:"key_inflight"`
...
type RuntimeController interface {
	...
	CooldownProvider(ctx context.Context, upstreamID uint, reason string)
	CooldownKeyModel(ctx context.Context, keyID uint, model, reason string, status int)
